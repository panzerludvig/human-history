...
**Overshoot tuned to ~18%**
The user judged the ~1% overshoot too small (wanted 10–20%). Cause: population and land responded on similar ~10-year timescales, so the system was near-critically damped. Overshoot size is governed by the ratio of growth speed to land-feedback lag, while the equilibrium depends only on the ratio of the two land timescales — so the land clocks were slowed (regen 12→33 yr, depletion 8→22 yr, same 1.5 ratio, R* unchanged, calibration intact) and growth quickened (max 2.8%/yr, saturating fast). Parameters were chosen by offline simulation before touching the game, then verified in game: peak 426 against capacity 365 (+17%) at year 33, settled at exactly 365 by year 153.
