...
// contact draws, and the world invention clock. Order matters because tech
// events change neighbours' rates mid-jump.
inline bool simulate(population::Field& pf, WorldState& ws, double now) {
    ...
    while (true) {
        ...
        if (kind == 1) {
            ...
            changed |= population::advance(s, effectiveK(s, t), t);
        } else if (kind == 2) {
            population::Settlement& s = pf.settlements[idx];
            if (!s.techFires) { redraw(pf, idx, ws, t); continue; }
            if (!s.aware) {
                s.aware = true;
                fprintf(stderr, "tech: settlement %d aware of farming, day %.0f\n", idx, t);
                redraw(pf, idx, ws, t);
                for (int j : pf.neighbours[idx])
                    if (!pf.settlements[j].aware) redraw(pf, j, ws, t);
            } else {
                startPractising(pf, idx, ws, t);
                fprintf(stderr, "tech: settlement %d starts farming, day %.0f\n", idx, t);
            }
            scheduleInvention(pf, ws, t);
            changed = true;
        } else {
            if (!ws.fires) { scheduleInvention(pf, ws, t); continue; }
            int wi = pickInventor(pf, ws, t);
            if (wi >= 0) {
                startPractising(pf, wi, ws, t);
