...
A list of concrete, actionable tasks — as distinct from [[Meta/Suggestions]], which are options to consider rather than things to do. Items can be added by Claude or the user. Once an item is done, remove it from this list (the Dev Log can record the outcome if it's worth preserving).
